...
<template>
  <div class="q-pa-xs">
    ...
    <div
      ...
      class="text-primary q-gutter-sm text-center"
    >
      <br />
      <br />
      <br />
      <div class="text-h6">Hi! What are you looking for today?</div>
      ...
      <div class="q-gutter-y-sm cart">
        ...
        <q-dialog v-model="dialog" persistent>
          <div class="q-pa-md">
            <q-card>
              <q-card-section>
                <q-item-label>Barcode not found in product data.</q-item-label>
              </q-card-section>
              <q-card-actions>
...
<script>
import productData from "../json/test.json";
...
export default {
  ...
  methods: {
    ...
    addItem() {
      if (this.barcode) {
        const barcodeExists = this.productData.some(
          (product) => product.gtin === this.barcode
        );
        if (barcodeExists) {
          const existingItemIndex = this.cartItems.findIndex(
            (item) => item.barcode === this.barcode
          );
